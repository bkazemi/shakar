...
def eval_args_node(
    ...
    if is_tree(args_node):
        return _eval_args(_flatten_args(args_node), frame, eval_fn)

    if isinstance(args_node, list):
        res: List[Node] = []
        for n in args_node:
            res.extend(_flatten_args(n))
        return _eval_args(res, frame, eval_fn)
...
def eval_args_node_with_named(
    ...
    if is_tree(args_node):
        return _eval_args_with_named(_flatten_args(args_node), frame, eval_fn)

    if isinstance(args_node, list):
        res: List[Node] = []
        for n in args_node:
            res.extend(_flatten_args(n))
        return _eval_args_with_named(res, frame, eval_fn)

    return [], {}, False


def _eval_args(nodes: List[Node], frame: Frame, eval_fn: EvalFn) -> List[ShkValue]:
    positional, named, _interleaved = _eval_args_with_named(nodes, frame, eval_fn)
    if named:
...
def _eval_args_with_named(
    nodes: List[Node], frame: Frame, eval_fn: EvalFn
) -> Tuple[List[ShkValue], Dict[str, ShkValue], bool]:
    ...
    for node in nodes:
        if _is_namedarg(node):
            ...
            named[key] = eval_anchor_scoped(value_expr, frame, eval_fn)
            continue
        ...
        if _is_raw_fieldfan(node):
            spread_val = eval_anchor_scoped(node, frame, eval_fn)
        ...
        positional.append(eval_anchor_scoped(node, frame, eval_fn))
...
def evaluate_index_operand(
    ...
    return eval_fn(expr_node, frame)


def apply_slice(
    recv: ShkValue, arms: List[Node], frame: Frame, eval_fn: EvalFn
) -> ShkValue:
    def arm_to_py(node: Node) -> Optional[int]:
        if tree_label(node) == "emptyexpr":
            return None

        value = eval_fn(node, frame)
...
def apply_index_operation(
    recv: ShkValue, op: Tree, frame: Frame, eval_fn: EvalFn
) -> ShkValue:
...
def apply_op(recv: EvalResult, op: Tree, frame: Frame, eval_fn: EvalFn) -> EvalResult:
    if isinstance(recv, FanContext):
        return apply_fan_op(recv, op, frame, apply_op=apply_op, eval_fn=eval_fn)
    ...
    if op_name in {"field", "fieldsel"}:
        ...
        result = apply_index_operation(recv, op, frame, eval_fn)
    elif op_name == "slicesel":
        result = apply_slice(recv, op.children, frame, eval_fn)
        ...
        result = _call_method(recv, op, frame, eval_fn)
    else:
...
def _call_method(recv: ShkValue, op: Tree, frame: Frame, eval_fn: EvalFn) -> ShkValue:
    method_name = _method_name_token(op.children[0])
    positional, named, interleaved = eval_args_node_with_named(
        op.children[1] if len(op.children) > 1 else None, frame, eval_fn
    )
...
def _valuefan(base: ShkValue, op: Tree, frame: Frame, eval_fn: EvalFn) -> ShkValue:
    """Evaluate value fanout `base.{...}` to an array of values."""
    return eval_valuefan(base, op, frame, eval_fn, apply_op)
