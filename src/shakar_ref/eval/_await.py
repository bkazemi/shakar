...
async def _await_any_async(
    entries: list[tuple[AwaitArm, ShkValue]],
) -> tuple[AwaitArm, ShkValue]:
...
async def _await_all_async(
    entries: list[tuple[AwaitArm, ShkValue]],
) -> list[tuple[AwaitArm, ShkValue]]:
...
def _collect_await_arms(list_node: Tree, prefix: str) -> list[AwaitArm]:
    ...
    for idx, arm_node in enumerate(tree_children(list_node)):
        ...
        if pos < len(children):
            ...
            if is_tree(candidate) and tree_label(candidate) in {
                "inlinebody",
                "indentblock",
            }:
                body_node = candidate
    ...
    if short_form is not None:
        ...
        if kind == "any":
            ...
            if body_node is not None:
                ...
                return run_body_with_subject(
                    body_node, frame, winner_value, eval_func, bindings
                )
...
def eval_await_any_call(n: Tree, frame: Frame, eval_func: EvalFunc) -> ShkValue:
    ...
    if trailing_body is not None and per_arm_bodies:
        raise ShakarRuntimeError(
            "await[any] cannot mix per-arm bodies with a trailing body"
        )
    ...
    if trailing_body is not None:
        bindings = {"winner": ShkString(winner_arm["label"])}
        return run_body_with_subject(
            trailing_body, frame, winner_value, eval_func, bindings
        )
...
def eval_await_all_call(n: Tree, frame: Frame, eval_func: EvalFunc) -> ShkValue:
    ...
    resolved = await_all_entries(entries)
    results_object: dict[str, ShkValue] = {
        arm["label"]: value for arm, value in resolved
    }
    trailing_body = _extract_trailing_body(tree_children(n))
