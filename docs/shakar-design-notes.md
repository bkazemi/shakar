...
- **Truthiness**: falsey = `nil`, `false`, zero numbers/durations/sizes, empty strings/arrays/sets/objects/paths/commands; truthy = non-empty of those. Fans, regexes, selectors, functions/methods, and type descriptors are invalid in scalar boolean contexts (type error).
...
- **Fans**: `fan { expr, ... }` (reserved keyword; not a valid identifier or property name). Evaluates elements left=>right into a `Fan`; property/method access broadcasts across elements and returns a `Fan`. Unary operators (`-`, `not`, `!`) broadcast element-wise over fans (e.g., `- fan { 1, 2 }` => `fan { -1, -2 }`). Binary value operators follow the same broadcast rule: `fan op scalar` and `scalar op fan` broadcast, while `fan op fan` zips by arity and errors on mismatch. Comparison operators follow the same broadcast/zip rule and return `Fan[Bool]`; chained comparisons combine per-item. General logical operators (`and` / `or`) remain scalar control-flow operators rather than fan-aware value operators. Prefix/postfix `++`/`--` broadcast over fans when each item is an assignable target (e.g., `++fan { a, b }` increments both; `state.{x, y}++` post-increments both fields). Fans are iterable (e.g., `for x in fan { ... }`). Fan literals can also be used as assignment lvalue heads when each item resolves to an assignable target (`=`, `.=`, and compound assignment). For concurrency, use `spawn` to create channels and `wait[all]` to join them. Modifiers like `fan[par] { ... }` are reserved but not implemented in v0.1.
...
- **Expression punning**: when an entry has no `:` and is not a bare identifier, the leading identifier becomes the key: `{score ** 2}` desugars to `{score: score ** 2}`. Works with any expression that starts with a bare IDENT — binary ops (`{count + 1}`), method chains (`{name.upper()}`), calls (`{f(x)}`), ternaries (`{flag ? a : b}`). Dot-chain expressions where the leading identifier is not the intended key require explicit form: `{score: user.score}`. Explicit and punned entries mix freely: `{score ** 2, label: "result"}`.
...
- **Increments**: prefix/postfix `++/--` on lvalues (numeric only). Postfix terminal and only at end of member/selector chain; prefix returns updated value, postfix the pre-update value. Both broadcast over fans: `++fan { a, b }` and `state.{x, y}++` increment each target and return a `Fan` of results.
...
- **Contexts**:
  ...
  - **Destructure patterns**: `a, ...rest := [1, 2, 3]` collects surplus values into `rest`. Must be last in the pattern list. See [Pattern defaults and rest](#assignment--mutation) above.
