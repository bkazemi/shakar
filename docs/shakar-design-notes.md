...
- **Fans**: `fan { expr, ... }` (reserved keyword; not a valid identifier or property name). Evaluates elements left=>right into a `Fan`; property/method access broadcasts across elements and returns a `Fan`. Binary value operators follow the same broadcast rule: `fan op scalar` and `scalar op fan` broadcast, while `fan op fan` zips by arity and errors on mismatch. Comparison operators follow the same broadcast/zip rule and return `Fan[Bool]`; chained comparisons combine per-item. General logical operators (`and` / `or`) remain scalar control-flow operators rather than fan-aware value operators. Fans are iterable (e.g., `for x in fan { ... }`). Fan literals can also be used as assignment lvalue heads when each item resolves to an assignable target (`=`, `.=`, and compound assignment). For concurrency, use `spawn` to create channels and `wait[all]` to join them. Modifiers like `fan[par] { ... }` are reserved but not implemented in v0.1.
